...
class Shin_MechaZombie : ShinDoom_Actor
{
	...
	See:
		...
		Loop;
	Missile:
		POSM AF 8 A_FaceTarget;
		POSM F 4 A_MechaAttack();
		POSM G 4 Bright A_FaceTarget;
		POSM F 4 A_MechaAttack();
		POSM G 4 Bright A_FaceTarget;
		POSM F 4 A_MechaAttack();
		POSM G 4 Bright A_FaceTarget;
...
Extend Class Shin_MechaZombie
{
	Void A_MechaAttack()
	{
		...
		{
			A_FaceTarget();
			A_SpawnProjectile("Shin_PlasmaBall1",30,11,0,CMF_BADPITCH);
		}
	}
}